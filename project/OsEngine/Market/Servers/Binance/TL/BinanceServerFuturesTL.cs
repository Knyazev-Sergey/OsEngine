...
namespace OsEngine.Market.Servers.Binance.TL
{
     /*enum FuturesType
    {
        USDT,
    ...
    public class BinanceServerFuturesTL : AServer
    {
        public BinanceServerFuturesTL(int uniqueNumber)
        {
            ServerNum = uniqueNumber;

            BinanceServerFuturesTLRealization realization = new BinanceServerFuturesTLRealization();
        ...
        {
            ((BinanceServerFuturesTLRealization)ServerRealization).HedgeMode = ((ServerParameterBool)ServerParameters[3]).Value;
        }
    }

    public class BinanceServerFuturesTLRealization : IServerRealization
    {
        #region 1 Constructor, Status, Connection

        public BinanceServerFuturesTLRealization()
        {
        ...
        public void Connect(WebProxy proxy = null)
        {
            ...
            {
                _baseUrl = "https://fapi.proxy-stage-02.mstrat.fund";
                wss_point = "wss://fstream.proxy-stage-02.mstrat.fund";
                type_str_selector = "fapi";
            }
            else if (((ServerParameterEnum)ServerParameters[2]).Value == "COIN-M")
            {
                _baseUrl = "https://dapi.proxy-stage-02.mstrat.fund";
                wss_point = "wss://dstream.proxy-stage-02.mstrat.fund";
        ...
        {
            get { return ServerType.BinanceFuturesTL; }
        }
        ...
        public string _baseUrl = "https://fapi.proxy-stage-02.mstrat.fund";

        public string wss_point = "wss://fstream.proxy-stage-02.mstrat.fund";
        ...
        private void UpdatePairs(SecurityResponse pairs)
        {
            foreach (var sec in pairs.symbols)
            {
                ...
                security.Exchange = ServerType.BinanceFuturesTL.ToString();
                security.Lot = 1;
        ...
        private void UpdatePortfolio(AccountResponseFutures portfs, bool IsUpdateValueBegin)
        {
            try
            {
                Portfolio myPortfolio = _portfolios.Find(p => p.Number == "BinanceFuturesTL");

                if (myPortfolio == null)
                {
                    Portfolio newPortf = new Portfolio();
                    newPortf.Number = "BinanceFuturesTL";
                ...
                foreach (var onePortf in portfs.assets)
                {
                    ...
                    newPortf.ValueCurrent =
                        onePortf.marginBalance.ToDecimal();
                    newPortf.PortfolioName = "BinanceFuturesTL";
        ...
        private void UpdateMyOrderAndMyOrders(string mes)
        {
            ...
            try
            {
                orderNumUser = Convert.ToInt32(order.c.ToString().Replace("x-PPY5PYpN", ""));
            }
            catch (Exception)
            {
            ...
            if (order.x == "NEW")
            {
                ...
                newOrder.ServerType = ServerType.BinanceFuturesTL;
                newOrder.PortfolioNumber = "BinanceFuturesTL";
        ...
        public void SendOrder(Order order)
        {
            ...
            try
            {
                ...
                param.Add("&newClientOrderId=", "x-PPY5PYpN" + order.NumberUser.ToString());
                param.Add("&quantity=",
            ...
            try
            {
                ...
                for (int i = 0; i < respOrders.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(orderOnBoardResp.clientOrderId) == false)
                    {
                        ...
                        {
                            newOrder.NumberUser =
                                Convert.ToInt32(orderOnBoardResp.clientOrderId.Replace("x-PPY5PYpN", ""));
                        }
                        catch
                        {
                            // ignore
            ...
            try
            {
                ...
                for (int i = 0; allOrders != null && i < allOrders.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(allOrders[i].clientOrderId) == false)
                    {
                        try
                        {
                            if (allOrders[i].clientOrderId.Replace("x-PPY5PYpN", "") == oldOrder.NumberUser.ToString())
                            {
