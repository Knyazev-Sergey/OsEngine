...
{
    [Bot("BreakLRChannel1")] // We create an attribute so that we don't write anything to the BotFactory
    public class BreakLRChannel : BotPanel
    {
        ...
        public BreakLRChannel(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            ReverseLogic = CreateParameter("ReverseLogic", true, "Base");
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            decimal periodUpLine = ChannelLR.DataSeries[0].Values[ChannelLR.DataSeries[0].Values.Count - PeriodLR.ValueInt];
            decimal periodDownLine = ChannelLR.DataSeries[2].Values[ChannelLR.DataSeries[2].Values.Count - PeriodLR.ValueInt];
            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                ...
                if (Regime.ValueString != "OnlyShort") // If the mode is not only short, then we enter long
                {
                    if (ReverseLogic.ValueBool)
                    {
                        ...
                        periodUpLine < _lastUpLine)
                        {
                            _tab.BuyAtLimit(GetVolume(), _tab.PriceBestAsk + GetSlippage(_tab.PriceBestAsk));
                        }
                    }
                    else
                    {
                ...
                if (Regime.ValueString != "OnlyLong") // If the mode is not only long, then we enter short
                {
                    if (ReverseLogic.ValueBool)
                    {
                        ...
                        periodDownLine > _lastDownLine)
                        {
                            _tab.SellAtLimit(GetVolume(), _tab.PriceBestBid - GetSlippage(_tab.PriceBestBid));
                        }
                    }
                    else
                    {
            ...
            for (int i = 0; openPositions != null && i < openPositions.Count; i++)
            {
                ...
                if (openPositions[i].Direction == Side.Buy) // If the direction of the position is purchase
                {
                    if (ReverseLogic.ValueBool)
                    {
                        if (_lastDownLine > lastPrice)
                        {
                            _tab.CloseAtLimit(openPositions[i], _tab.PriceBestBid - GetSlippage(_tab.PriceBestBid), openPositions[i].OpenVolume);
                        }                        
                    }
                    else
                    {
                ...
                else // If the direction of the position is sale
                {
                    if (ReverseLogic.ValueBool)
                    {
                        if (_lastUpLine < lastPrice)
                        {
                            _tab.CloseAtLimit(openPositions[i], _tab.PriceBestAsk + GetSlippage(_tab.PriceBestAsk), openPositions[i].OpenVolume);
                        }
                    }
                    else
                    {
