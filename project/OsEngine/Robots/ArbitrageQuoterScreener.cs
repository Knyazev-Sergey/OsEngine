    ...
    {       
        ...
        private DataGridView GridResultPosition()
        {
            ...
            for (int i = 1; i < 32; i++)
            {
                ...
                column.Width = 40; // Устанавливаем начальную ширину
        ...
        /*private ObservableCollection<ResultTable> _resultTables;
        ...
        private DataGridView GridResultPosition()
        {
            ...
            string[] listName = ["Exchange", "Tiker", "Direction", "Periodicity", "OpenVolume", "OpenPrice", "OpenSumm"];

            for (int i = 0; i < listName.Length; i++)
            {
                DataGridViewColumn column = new DataGridViewTextBoxColumn { DataPropertyName = listName[i] };
                column.SortMode = DataGridViewColumnSortMode.NotSortable;
                column.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                column.DefaultCellStyle.SelectionBackColor = newGrid.DefaultCellStyle.BackColor;
                column.DefaultCellStyle.SelectionForeColor = newGrid.DefaultCellStyle.ForeColor;
            ...
            _resultTables = new ObservableCollection<ResultTable>();
            newGrid.DataSource = _resultTables;

            AddStringsInResultTable();

            newGrid.DataError += (s, e) =>
            {
        ...
        private void AddStringsInResultTable()
        {
            _resultTables.Add(new ResultTable { Exchange = "Позиция", OpenVolume = "Открытая позиция" });
            _resultTables.Add(new ResultTable { Exchange = "Биржа", Tiker = "Инструмент", Direction = "Направление", Periodicity = "Периодичность", OpenVolume = "Кол-во", OpenPrice = "Цена", OpenSumm = "Сумма" });
        }*/
                ...
                for (int j = 0; j < TabsScreener[i].Tabs.Count; j++)
                {
                    ...
                    if (tab.PositionsAll.Count != 0)
                    {
                        ...
                        if (pos.CloseOrders != null && pos.CloseOrders.Count > 0)
                        {
                            ...
                            result.CloseSum = Math.Round(pos.MaxVolume * tab.Security.Lot * pos.ClosePrice, 2);
                            result.RealizPL = Math.Round((result.CloseSum - result.OpenSum) * result.CloseVolume, 4);
